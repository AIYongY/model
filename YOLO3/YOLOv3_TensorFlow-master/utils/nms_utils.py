...
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.5, iou_thresh=0.5):
    ...
    max_boxes = tf.constant(max_boxes, dtype='int32')#40 和默认值不一样
    ...
    #greater_equal(x,y,name=None ) 返回 (x > = y) 元素的真值.   0 1值
    #mask  [N*(13 * 13 + 26 * 26 + 52 * 52) * 3, class_num]   0 1值  大于  0.4 为1   预测类别的分数
    ...
    for i in range(num_classes):
        ...
        # boxes shape[N*(13*13+26*26+52*52)*3, 4]
        #mask  [N*(13 * 13 + 26 * 26 + 52 * 52) * 3, class_num]  0 1
        filter_boxes = tf.boolean_mask(boxes, mask[:,i])
        #有80个类别，每个类别 有N*(13 * 13 + 26 * 26 + 52 * 52) * 3多个值 里面为1 就说检测到该类别
        #返回的时  检测到物体的哪个框  四个坐标
        # 遍历类别
        ...
        nms_indices = tf.image.non_max_suppression(boxes=filter_boxes,
                                                   scores=filter_score,
                                                   max_output_size=max_boxes,#40
                                                   iou_threshold=iou_thresh, name='nms_indices')#0.5
        #这个函数是  非极大值抑制
        #保留概率最大的那个，去除掉与这个概率最大的边界框的IoU大于一个阙值的其余边界框。这个过程就成为非最大值抑制=NMS
        #iou_thresh是重叠的区域
        #40是 最后留下来 最多40个
