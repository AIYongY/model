...
def net_evaluation(sess, loss, accuracy, val_images_batch, val_labels_batch, val_nums):
    """
    测试网络的精确度，所有测试集的batch都测试
    return mean_loss, mean_acc   平均损失，平均精度
    """
    val_max_steps = int(val_nums / batch_size)#val_nums总测试集数量
    val_losses = []
    val_accs = []
...
def step_train(train_op, loss, accuracy,
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps + 1):# max_steps = 100000  # 迭代次数
            ...
            # 测试也是batch输入，这里输入仅一个batch
            if i % train_log_step == 0:# train_log_step = 100 就是迭代100次打印
                #测试的时候将feed_dict的数据改成测试集的数据，keep_prob要等与1，is_training: False
                ...
                #datetime.now()就是现在的时间
            # val测试(测试全部val数据)     测试所有的batch 测试集也有batch
            if i % val_log_step == 0:# val_log_step = 500
            ...
            #max_steps = 100000  # 迭代次数  每2000次保存
...
"""
path = "aaa"
best_models = os.path.join(path, 'best_models_{}_{:.4f}.ckpt'.format("hello", 4)) 4f是小数点4位
#aaa\best_models_hello_4.0000.ckpt
print(best_models)
